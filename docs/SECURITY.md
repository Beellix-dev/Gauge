...
v0.1.1 起，Gauge 将观察到的本地 Codex 文件授权保存为独立账号副本，按用户和工作空间身份匹配已有条目。保存前限制临时文件 ACL，并复查源文件和已有副本未变化，再原子替换；此自动保存不会写回 Codex 的共用登录目录。同一 Windows 用户可读取普通授权文件。检查期间正在执行 Gauge 查询或切换时，登录变化轮询会等待任务结束。
